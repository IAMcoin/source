...
uint256 hashGenesisBlock("0x00000d5d7faa225ed062ee48aa92411881e9d62eafc52d2f6dae3b7b2f3c8aca");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // IAMcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "IAMcoin Signed Message:\n";
...
{
    // IAMcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // IAMcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // IAMcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        assert(block.hashMerkleRoot == uint256("0xd269070f055cff2945e4195d02e01c1f0fa30d102168c20e36ff0dd1a1e3b25e"));
...
unsigned char pchMessageStart[4] = { 0xe2, 0xc1, 0xd3, 0xdb }; // IAMcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("IAMcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("IAMcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static IAMcoinMiner(CWallet *pwallet)
{
    printf("IAMcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("IAMcoin-miner");
    ...
    try { loop {
        ...
        printf("Running IAMcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("IAMcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&IAMcoinMiner, pwallet));
}
